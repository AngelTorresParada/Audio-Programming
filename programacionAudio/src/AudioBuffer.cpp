...
AudioBuffer * AudioBuffer::load(const char * _filename) {
	...
	if (extractExt(_filename) == "wav") {
		...
		if (audioFormat != 1) return nullptr;
	...
	else if (extractExt(_filename) == "ogg") {
		AudioBuffer *aBuffer = new AudioBuffer();
		stb_vorbis *vorbis = stb_vorbis_open_filename(_filename, nullptr, nullptr);
		if (! vorbis) return nullptr;

		stb_vorbis_info info = stb_vorbis_get_info(vorbis);
		size_t oggLength = stb_vorbis_stream_length_in_samples(vorbis) * info.channels;
		int16_t *buffer = new int16_t[oggLength];

		stb_vorbis_get_samples_short_interleaved(vorbis, info.channels, buffer, oggLength);
		...
		alBufferData(aBuffer->buffer, format, buffer, oggLength, info.sample_rate);
...
std::string AudioBuffer::extractExt(const std::string &_filename) {
		return _filename.substr(_filename.rfind('.') + 1, _filename.length() - _filename.rfind('.') - 1);
}
